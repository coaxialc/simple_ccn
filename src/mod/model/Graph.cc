...
namespace ns3
{
	...
	void Graph::DoDispose(void)
	{

	}

	void Graph::setGraph(map<uint32_t, set<uint32_t> > g)
	{
		graph=g;
	...
	bool Graph::isItCoreNode(uint32_t d)
	{
		if( graph[d].size()>1 )
		{
			return true;
	...
	set<uint32_t> Graph::getNodesWithDegree(uint32_t d)
	{
		set<uint32_t> result;
		map< uint32_t , set < uint32_t > >::iterator it;
	...
	void Graph::Dijkstra(uint32_t source)
	{
		...
		//uint32_t matrix [graph.size()+1][graph.size()+1];
		...
		for(it = graph.begin(); it != graph.end(); it++)
		{
			...
			//	std::cout<<"grafei sto matrix-------------------"<<std::endl;
			}
		}
		...
		for(uint32_t i=0;i<graph.size()+1;i++)
		{
			for(uint32_t j=0;j<graph.size()+1;j++)
			{
				if(i!=0 && j!=0) std::cout<<matrix[i][j];
